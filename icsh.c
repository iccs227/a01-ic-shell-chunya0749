...
void externalRunning(char **args, int background) {
    ...
    if (pid < 0) {
        ...
        if (background == 1) {
            ...
            fg_pid = pid;  // Track foreground job PID
            fgJob = 1;

            // Wait for the job to either exit OR stop (Ctrl+Z)
            waitpid(pid, &status, WUNTRACED);

            if (WIFSTOPPED(status)) {
                // Job was stopped by Ctrl+Z (SIGTSTP)
                printf("Foreground job suspended\n");
                addJob(pid, flattenArgs(args), 1);  // 1 = stopped
            }
...
void printJobList() {
    ...
    for (int i = 1; i <= jobID; i++) {
        if (strcmp(jobList[i].Process_status, "Running") == 0 ||
            strcmp(jobList[i].Process_status, "Stopped") == 0) {
